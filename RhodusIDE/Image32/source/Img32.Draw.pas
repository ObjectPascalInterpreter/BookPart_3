...
type
  ...
  protected
    ...
    function Initialize(imgBase: Pointer; imgWidth, imgHeight, pixelSize: integer): Boolean; overload; virtual;
  ...
  protected
    ...
    function Initialize(targetImage: TImage32): Boolean; override;
  public
    constructor Create(tileFillStyle: TTileFillStyle = tfsRepeat; brushImage: TImage32 = nil);
  ...
  public
    constructor Create;
    procedure SetParameters(startColor, endColor: TColor32; gradFillStyle: TGradientFillStyle = gfsClamp); virtual;
  ...
  protected
    ...
    function Initialize(targetImage: TImage32): Boolean; override;
  public
    procedure SetParameters(const startPt, endPt: TPointD; startColor, endColor: TColor32;
  ...
  protected
    ...
    function Initialize(targetImage: TImage32): Boolean; override;
  public
    procedure SetParameters(const focalRect: TRect; innerColor, outerColor: TColor32;
  ...
  protected
    ...
    function Initialize(targetImage: TImage32): Boolean; override;
  public
    procedure SetParameters(const ellipseRect: TRect; const focus: TPoint; innerColor, outerColor: TColor32;
      gradientFillStyle: TGradientFillStyle = gfsClamp); reintroduce;
  end;
...
procedure DrawLine(img: TImage32; const pt1, pt2: TPointD; lineWidth: double; color: TColor32); overload;
procedure DrawLine(img: TImage32; const line: TPathD; lineWidth: double; color: TColor32; endStyle: TEndStyle;
  joinStyle: TJoinStyle = jsAuto; miterLimit: double = 2); overload;
procedure DrawLine(img: TImage32; const line: TPathD; lineWidth: double; renderer: TCustomRenderer; endStyle: TEndStyle;
  joinStyle: TJoinStyle = jsAuto; miterLimit: double = 2); overload;
procedure DrawLine(img: TImage32; const lines: TPathsD; lineWidth: double; color: TColor32; endStyle: TEndStyle;
  joinStyle: TJoinStyle = jsAuto; miterLimit: double = 2); overload;
procedure DrawLine(img: TImage32; const lines: TPathsD; lineWidth: double; renderer: TCustomRenderer; endStyle: TEndStyle;
  joinStyle: TJoinStyle = jsAuto; miterLimit: double = 2); overload;

procedure DrawInvertedLine(img: TImage32; const line: TPathD; lineWidth: double; endStyle: TEndStyle;
  joinStyle: TJoinStyle = jsAuto); overload;
procedure DrawInvertedLine(img: TImage32; const lines: TPathsD; lineWidth: double; endStyle: TEndStyle;
  joinStyle: TJoinStyle = jsAuto); overload;

procedure DrawDashedLine(img: TImage32; const line: TPathD; dashPattern: TArrayOfInteger; patternOffset: PDouble;
  lineWidth: double; color: TColor32; endStyle: TEndStyle; joinStyle: TJoinStyle = jsAuto); overload;
procedure DrawDashedLine(img: TImage32; const lines: TPathsD; dashPattern: TArrayOfInteger; patternOffset: PDouble;
  lineWidth: double; color: TColor32; endStyle: TEndStyle; joinStyle: TJoinStyle = jsAuto); overload;
procedure DrawDashedLine(img: TImage32; const line: TPathD; dashPattern: TArrayOfInteger; patternOffset: PDouble;
  lineWidth: double; renderer: TCustomRenderer; endStyle: TEndStyle; joinStyle: TJoinStyle = jsAuto); overload;
procedure DrawDashedLine(img: TImage32; const lines: TPathsD; dashPattern: TArrayOfInteger; patternOffset: PDouble;
  lineWidth: double; renderer: TCustomRenderer; endStyle: TEndStyle; joinStyle: TJoinStyle = jsAuto); overload;

procedure DrawInvertedDashedLine(img: TImage32; const line: TPathD; dashPattern: TArrayOfInteger; patternOffset: PDouble;
  lineWidth: double; endStyle: TEndStyle; joinStyle: TJoinStyle = jsAuto); overload;
procedure DrawInvertedDashedLine(img: TImage32; const lines: TPathsD; dashPattern: TArrayOfInteger; patternOffset: PDouble;
  lineWidth: double; endStyle: TEndStyle; joinStyle: TJoinStyle = jsAuto); overload;
...
procedure DrawPolygon_ClearType(img: TImage32; const polygons: TPathsD; fillRule: TFillRule; color: TColor32;
  backColor: TColor32 = clWhite32);
...
procedure Rasterize(const paths: TPathsD; const clipRec: TRect; fillRule: TFillRule; renderer: TCustomRenderer);
...
procedure ApplyClearType(img: TImage32; textColor: TColor32 = clBlack32; bkColor: TColor32 = clWhite32);
const
  ...
  for h := 0 to img.Height - 1 do
    begin
      ...
      while w < img.Width do
        begin
          dst.R := primeTbl[srcArr[w].a] + nearTbl[srcArr[w - 1].a] + FarTbl[srcArr[w - 2].a] + nearTbl[srcArr[w + 1].a] +
            FarTbl[srcArr[w + 2].a];
          inc(w);
          dst.G := primeTbl[srcArr[w].a] + nearTbl[srcArr[w - 1].a] + FarTbl[srcArr[w - 2].a] + nearTbl[srcArr[w + 1].a] +
            FarTbl[srcArr[w + 2].a];
          inc(w);
          dst.b := primeTbl[srcArr[w].a] + nearTbl[srcArr[w - 1].a] + FarTbl[srcArr[w - 2].a] + nearTbl[srcArr[w + 1].a] +
            FarTbl[srcArr[w + 2].a];
          inc(w);
          dst.a := 255;
          inc(dst);
...
begin
  dec(colorCnt);
  if Odd(Round(d)) then
    result := Round((1 - frac(d)) * colorCnt)
  else
...
function MakeColorGradient(const gradColors: TArrayOfGradientColor; len: integer): TArrayOfColor32;
var
  i, j, lenC: integer;
...
procedure AllocateScanlines(const polygons: TPathsD; var scanlines: TArrayOfScanline; clipBottom, clipRight: integer);
var
              ...
              if (y2 >= 0) and (y1 <= clipBottom) then
                begin
                  ...
                  if y2 >= clipBottom then
                    inc(scanlines[clipBottom].fragCnt)
                  else
...
procedure SplitEdgeIntoFragments(const pt1, pt2: TPointD; const scanlines: TArrayOfScanline; const clipRec: TRect);
var
...
begin
  ...
  if bot.y > maxY then
    begin
      ...
      if (bot.x > maxX) then
        Exit; // nb: no clipping on the left
      bot.y := maxY;
    end;
  if top.y < 0 then
    begin
  ...
  if not assigned(psl.fragments) then
    Exit; // a very rare event
{$IFDEF MemCheck}
  if psl.fragCnt = psl.total then
    raise Exception.Create(sMemCheckError);
{$ENDIF}
...
procedure InitializeScanlines(var polygons: TPathsD; const scanlines: TArrayOfScanline; const clipRec: TRect);
var
  i, j, highJ: integer;
  pt1, pt2: PPointD;
begin
...
procedure ProcessScanlineFragments(var scanline: TScanLine; var buffer: TArrayOfDouble);
var
...
begin
  ...
  for i := 0 to high(scanlines) do
    begin
          ...
          case fillRule of
            frEvenOdd:
              begin
                ...
                if aa > 1275 then
                  byteBuffer[j] := Min(255, (2550 - aa) shr 2)
                else // /4
      ...
      renderer.RenderProc(clipRec2.Left + xli, clipRec2.Left + xri, clipRec2.top + i, @byteBuffer[xli]);
...
function TCustomRenderer.Initialize(imgBase: Pointer; imgWidth, imgHeight, pixelSize: integer): Boolean;
begin
  fImgBase := imgBase;
  fImgWidth := imgWidth;
...
type
  THackedImage32 = class(TImage32); // exposes protected Changed method.
...
begin
  ...
  for i := x1 to x2 do
    begin
      if Ord(alpha^) > 127 then
        dst^ := fColor; // ie no blending
      inc(dst);
      inc(alpha);
    end;
end;
...
constructor TImageRenderer.Create(tileFillStyle: TTileFillStyle; brushImage: TImage32);
begin
...
begin
  ...
  for i := x1 to x2 do
    begin
      pDst^ := BlendToAlpha(pDst^, MulBytes(pBrush.a, Ord(alpha^)) shl 24 or (pBrush.color and $FFFFFF));
      inc(pDst);
      inc(alpha);
...
procedure TCustomGradientRenderer.SetParameters(startColor, endColor: TColor32; gradFillStyle: TGradientFillStyle = gfsClamp);
begin
...
begin
  ...
  i := 1;
  while (i < len - 1) and (fGradientColors[i].offset <= offsetFrac) do
    inc(i);
  SetLength(fGradientColors, len + 1);
  Move(fGradientColors[i], fGradientColors[i + 1], (len - i) * SizeOf(TGradientColor));
...
procedure TLinearGradientRenderer.SetParameters(const startPt, endPt: TPointD; startColor, endColor: TColor32;
  gradFillStyle: TGradientFillStyle);
begin
...
begin
  ...
  for i := x1 to x2 do
    begin
      ...
      pDst^ := BlendToAlpha(pDst^, MulBytes(color.a, Ord(alpha^)) shl 24 or (color.color and $FFFFFF));
      inc(pDst);
      inc(alpha);
    end;
end;
...
procedure TRadialGradientRenderer.SetParameters(const focalRect: TRect; innerColor, outerColor: TColor32;
...
procedure TSvgRadialGradientRenderer.SetParameters(const ellipseRect: TRect; const focus: TPoint;
  innerColor, outerColor: TColor32; gradientFillStyle: TGradientFillStyle = gfsClamp);
var
  radii: TPointD;
begin
  ...
  case gradientFillStyle of
    gfsMirror:
      fBoundsProcD := MirrorD;
    gfsRepeat:
      fBoundsProcD := RepeatD;
  else
    fBoundsProcD := ClampD;
  end;

  fColorsCnt := 0;
  if IsEmptyRect(ellipseRect) then
    Exit;
...
procedure TBarycentricRenderer.SetParameters(const a, b, c: TPointD; c1, c2, c3: TColor32);
begin
...
procedure DrawPoint(img: TImage32; const pt: TPointD; radius: double; color: TColor32);
var
  path: TPathD;
begin
  if radius <= 1 then
    path := Rectangle(pt.x - radius, pt.y - radius, pt.x + radius, pt.y + radius)
  else
...
procedure DrawPoint(img: TImage32; const pt: TPointD; radius: double; renderer: TCustomRenderer);
var
  path: TPathD;
begin
  path := Ellipse(RectD(pt.x - radius, pt.y - radius, pt.x + radius, pt.y + radius));
...
procedure DrawPoint(img: TImage32; const points: TPathD; radius: double; color: TColor32);
var
  i: integer;
begin
...
procedure DrawPoint(img: TImage32; const paths: TPathsD; radius: double; color: TColor32);
var
  i: integer;
begin
...
procedure DrawLine(img: TImage32; const pt1, pt2: TPointD; lineWidth: double; color: TColor32);
var
  lines: TPathsD;
begin
  SetLength(lines, 1);
...
procedure DrawLine(img: TImage32; const line: TPathD; lineWidth: double; color: TColor32; endStyle: TEndStyle;
  joinStyle: TJoinStyle; miterLimit: double);
var
  lines: TPathsD;
begin
  SetLength(lines, 1);
...
begin
  if not assigned(lines) then
    Exit;
  if (lineWidth < MinStrokeWidth) then
    lineWidth := MinStrokeWidth;
...
var
  lines2: TPathsD;
begin
  if (not assigned(lines)) or (not assigned(renderer)) then
    Exit;
  if (lineWidth < MinStrokeWidth) then
    lineWidth := MinStrokeWidth;
  lines2 := Outline(lines, lineWidth, joinStyle, endStyle, miterLimit);
...
begin
  ...
  for i := 0 to high(lines) do
    DrawDashedLine(img, lines[i], dashPattern, patternOffset, lineWidth, color, endStyle, joinStyle);
end;
// ------------------------------------------------------------------------------

procedure DrawDashedLine(img: TImage32; const line: TPathD; dashPattern: TArrayOfInteger; patternOffset: PDouble;
  lineWidth: double; renderer: TCustomRenderer; endStyle: TEndStyle; joinStyle: TJoinStyle);
var
  i: integer;
  lines: TPathsD;
begin
  if (not assigned(line)) or (not assigned(renderer)) then
    Exit;
  if (lineWidth < MinStrokeWidth) then
    lineWidth := MinStrokeWidth;
...
begin
  if not assigned(lines) then
    Exit;
  for i := 0 to high(lines) do
    DrawDashedLine(img, lines[i], dashPattern, patternOffset, lineWidth, renderer, endStyle, joinStyle);
end;
// ------------------------------------------------------------------------------
...
begin
  if not assigned(lines) then
    Exit;
  for i := 0 to high(lines) do
    DrawInvertedDashedLine(img, lines[i], dashPattern, patternOffset, lineWidth, endStyle, joinStyle);
end;
// ------------------------------------------------------------------------------

procedure DrawPolygon(img: TImage32; const polygon: TPathD; fillRule: TFillRule; color: TColor32);
var
  polygons: TPathsD;
begin
...
procedure DrawPolygon(img: TImage32; const polygon: TPathD; fillRule: TFillRule; renderer: TCustomRenderer);
var
  polygons: TPathsD;
begin
  if (not assigned(polygon)) or (not assigned(renderer)) then
    Exit;
  SetLength(polygons, 1);
  polygons[0] := polygon;
  if renderer.Initialize(img) then
...
procedure DrawPolygon(img: TImage32; const polygons: TPathsD; fillRule: TFillRule; color: TColor32);
var
  cr: TCustomRenderer;
begin
  if not assigned(polygons) then
    Exit;
  if img.AntiAliased then
    cr := TColorRenderer.Create(color)
  else
...
procedure DrawPolygon(img: TImage32; const polygons: TPathsD; fillRule: TFillRule; renderer: TCustomRenderer);
begin
  if (not assigned(polygons)) or (not assigned(renderer)) then
    Exit;
  if renderer.Initialize(img) then
    begin
      Rasterize(polygons, img.bounds, fillRule, renderer);
...
procedure ErasePolygon(img: TImage32; const polygon: TPathD; fillRule: TFillRule);
var
  polygons: TPathsD;
begin
  if not assigned(polygon) then
...
procedure ErasePolygon(img: TImage32; const polygons: TPathsD; fillRule: TFillRule);
var
  er: TEraseRenderer;
begin
...
begin
  len := Length(mask);
  if (len = 0) or (len <> img.Width * img.Height) then
    Exit;
  pc := img.PixelBase;
  pb := @mask[0];
  for i := 0 to len - 1 do
...
{$IFDEF PBYTE}
      if pb^ > 0 then
        pc^ := color or pb^ shl 24
      else
        pc^ := clNone32;
{$ELSE}
      if pb^ > #0 then
        pc^ := color or Ord(pb^) shl 24
      else
        pc^ := clNone32;
{$ENDIF}
      inc(pc);
